...
// MERGE RESOLVED: fe711489 version kept entirely.
// Only addition: canDelete prop gates Archive/Restore — managers only.
...
export default function ArtistManager({
  ...
  error: externalError,
  // ADDED: canDelete gates Archive/Restore — only managers get these buttons.
  // Edit is always available to anyone whose dept has canEdit: ["artists"].
  canDelete = true,
}) {
  ...
  // ── Archived Panel — only rendered for managers (canDelete) ──────────────
  const ArchivedPanel = () => (
  ...
  return (
    <div className="artist-manager">
      ...
      {archiveError && (
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            ...
            <button onClick={() => setArchiveError(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#991b1b", fontSize: 16, marginLeft: 8, lineHeight: 1 }}>×</button>
          </div>
      ...
      {/* Archived panel — managers only */}
      {canDelete && showArchived && <ArchivedPanel />}
      ...
      {/* Restore confirmation modal — managers only */}
      {restoreTarget && canDelete && (
        <div className="um-overlay" onClick={() => setRestoreTarget(null)}>
          <div className="um-modal" style={{ maxWidth: 400 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="um-modal-footer">
              ...
              <button className="um-save-btn" style={{ background: "#16a34a" }} onClick={handleRestoreConfirm}>
                ↩ Restore
              </button>
            </div>
          </div>
