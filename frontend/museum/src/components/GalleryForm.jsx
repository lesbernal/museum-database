...
const CLIMATE_OPTIONS = [
  { value: "", label: "Select climate control range" },
  { value: "60-65", label: "60–65°F (Cold Storage)" },
  { value: "65-70", label: "65–70°F (Cool / Sensitive Works)" },
  { value: "70-75", label: "70–75°F (Standard Gallery)" },
  { value: "75-80", label: "75–80°F (Warm / Textiles)" },
  { value: "none",  label: "Not Climate Controlled" },
];
...
export default function GalleryForm({ onSubmit, initialData = null, onCancel, isOpen = true }) {
  ...
  useEffect(() => {
    if (initialData) {
      setForm({
        ...
        climate_control_range: initialData.climate_control_range || "",
  ...
  const validateForm = () => {
    ...
    if (!form.climate_control_range) newErrors.climate_control_range = "Climate control range is required";
          ...
          <form onSubmit={handleSubmit} className="gallery-form">
            <div className="form-grid">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <select
                    name="climate_control_range"
                    value={form.climate_control_range}
                    onChange={handleChange}
                    className={errors.climate_control_range ? "error" : ""}
                  >
                    {CLIMATE_OPTIONS.map(opt => (
                      <option key={opt.value} value={opt.value}>{opt.label}</option>
                    ))}
                  </select>
                  {errors.climate_control_range && <span className="error-message">{errors.climate_control_range}</span>}
                </div>
                ...
                {exhibitions.length === 0 ? (
                  ...
                  <div className="checkbox-list">
                    {exhibitions.map(exhibition => {
                      const checked = form.exhibition_ids.map(String).includes(String(exhibition.exhibition_id));
                      return (
            ...
            <div className="form-actions">
              ...
              <button type="submit" className="submit-btn" disabled={isSubmitting}>
                {isSubmitting ? "Saving..." : (initialData ? "Update Gallery" : "Add Gallery")}
              </button>
            </div>
          </form>
        </div>
